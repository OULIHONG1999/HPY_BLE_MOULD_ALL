...
void user_main()
{
    ...
    // 初始化led
    add_led(led_port, LED_BLINK_SLOW, LED_BLINK); // 初始化为闪烁
    // 检测蓝牙是否连接
    sys_timer_add(NULL, led_blink_timer, 10);
    sys_timer_add(NULL, ble_work_state_check_timer, 100);
    sys_timer_add(NULL, send_test_timer, 1000);
}
