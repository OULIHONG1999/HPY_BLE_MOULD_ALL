 ...
 * @LastEditTime: 2024-03-30 15:31:27
 * @FilePath: \蓝牙拓展模块\apps\spp_and_le\examples\hpy_ble_module\applications\user_main.c
 * @Description:
 *   初始化各个模块内容
 * 1 初始化蓝牙部分的信息
 *      1) 设备id
...
void send_uart_data_timer()
{
    ...
    else
    {
        if (port_cmd[flag + 4] != 0)
        {
            ...
            data_len = data_cmd[flag][5] + 7;
            data_cmd[flag][data_len - 1] = 0xff;
            // 计算crc
...
void user_main_thread(void *arg)
{
    ...
    // 创建定时器，发送串口数据
    sys_hi_timer_add(NULL, send_uart_data_timer, 30);
    while (1)
    {
        wdt_clear();
        os_time_dly(1);
